...
namespace MaroonSeal.Maths.Interpolation {
    [System.Serializable]
    public class CompositeLerpPath<TPath> : List<TPath>, ILerpPath where TPath : ILerpPath {
    ...
    [System.Serializable]
    public class CompositeLerpPath2D<TPath> : List<TPath>, ILerpPath2D where TPath : ILerpPath2D {
        #region ILerpPath2D
        public Vector2 LerpAlongPath(float _time) {
            if (Count <= 0) { return Vector2.zero; }
            _time *= this.Count;
            int index = (int)Mathf.Clamp(_time, 0, this.Count-1);
            return this[index].LerpAlongPath(_time - index);
        ...
        Vector3 ILerpPath.LerpAlongPath(float _time) { return LerpAlongPath(_time); }
        #endregion
    }
